...
public class Bookshelf extends JPanel implements GLEventListener {
    ...
    private float shelfColorR = 0.4f, shelfColorG = 0.2f, shelfColorB = 0.1f;
    private float shelfX = 0, shelfY = -1.2f, shelfZ = 0;
    ...
    public Bookshelf() {
        ...
        // Mouse listeners for drag/rotate
        ...
        canvas.addMouseMotionListener(new MouseMotionAdapter() {
            public void mouseDragged(MouseEvent e) {
                ...
                if (isRotating && !is2DView) {
                    ...
                    shelfX += dx * 0.01f; shelfZ -= dy * 0.01f;
                }
        ...
        canvas.addMouseWheelListener(e -> {
            shelfY += e.getWheelRotation() * 0.1f;
            canvas.repaint();
        });

        // UI controls
        JPanel controls = new JPanel();
        JSlider r = new JSlider(0, 255, (int)(shelfColorR*255));
        r.addChangeListener(e -> { shelfColorR = r.getValue()/255f; canvas.repaint(); });
        controls.add(new JLabel("R:")); controls.add(r);

        JSlider g = new JSlider(0, 255, (int)(shelfColorG*255));
        g.addChangeListener(e -> { shelfColorG = g.getValue()/255f; canvas.repaint(); });
        controls.add(new JLabel("G:")); controls.add(g);

        JSlider b = new JSlider(0, 255, (int)(shelfColorB*255));
        b.addChangeListener(e -> { shelfColorB = b.getValue()/255f; canvas.repaint(); });
        ...
        reset.addActionListener(e -> {
            rotateX=rotateY=0;
            shelfX=shelfZ=0; shelfY=-1.2f;
            isDaytime = is2DView = autoScale = shading = autoRotate = false;
            shelfColorR=0.4f; shelfColorG=0.2f; shelfColorB=0.1f;
    ...
    public void display(GLAutoDrawable d) {
        ...
        // Draw Room Floor (same as before)...
        ...
        gl.glPushMatrix();
        gl.glTranslatef(shelfX,shelfY,shelfZ);

        // Outer Frame
        gl.glColor3f(shelfColorR, shelfColorG, shelfColorB);
        gl.glPushMatrix();
        gl.glScalef(1.5f*scale,2.5f*scale,0.5f*scale);
        glut.glutWireCube(1);
        gl.glPopMatrix();
        ...
        for (int i=0; i<3; i++) {
            gl.glPushMatrix();
            gl.glTranslatef(0,0.5f - i,0);
            gl.glScalef(1.5f*scale,0.1f,0.5f*scale);
            glut.glutSolidCube(1);
            gl.glPopMatrix();
        }
        gl.glPopMatrix();
    ...
    public static void main(String[] args) {
        JFrame frame = new JFrame("3D Bookshelf");
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        frame.setSize(800,600);
        frame.setContentPane(new Bookshelf());
