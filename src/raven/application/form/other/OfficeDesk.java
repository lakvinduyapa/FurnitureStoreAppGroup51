...
public class OfficeDesk extends JPanel implements GLEventListener {
    ...
    public OfficeDesk() {
        ...
        canvas.addMouseListener(new MouseAdapter() {
            public void mousePressed(MouseEvent e)  { isDragging = true;  lastX = e.getX(); lastY = e.getY(); }
        ...
        control.add(new JLabel("Room Width:"));  control.add(widthField);
        control.add(new JLabel("Room Length:")); control.add(lengthField);
        ...
        control.add(new JLabel("Room Shape:")); control.add(shapeBox);

        // Desk color sliders
        JSlider rS = new JSlider(0,255,(int)(deskColorR*255)),
                gS = new JSlider(0,255,(int)(deskColorG*255)),
                bS = new JSlider(0,255,(int)(deskColorB*255));
        rS.addChangeListener(e->{ deskColorR = rS.getValue()/255f; canvas.repaint(); });
        gS.addChangeListener(e->{ deskColorG = gS.getValue()/255f; canvas.repaint(); });
        bS.addChangeListener(e->{ deskColorB = bS.getValue()/255f; canvas.repaint(); });
        control.add(new JLabel("R:")); control.add(rS);
        control.add(new JLabel("G:")); control.add(gS);
        control.add(new JLabel("B:")); control.add(bS);

        // Shading toggle
        JCheckBox shadeToggle = new JCheckBox("Enable Shading");
        shadeToggle.addActionListener(e -> { shadingEnabled = shadeToggle.isSelected(); canvas.repaint(); });
        control.add(shadeToggle);

        // Scale-fit toggle
        JCheckBox scaleToggle = new JCheckBox("Scale Fit");
        scaleToggle.addActionListener(e -> { scaleFitEnabled = scaleToggle.isSelected(); canvas.repaint(); });
        control.add(scaleToggle);

        // Day/Night
        JButton bgBtn = new JButton("Day/Night");
        bgBtn.addActionListener(e -> { isDaytime = !isDaytime; canvas.repaint(); });
        control.add(bgBtn);

        // 2D/3D toggle
        JButton viewBtn = new JButton("Toggle 2D/3D");
        viewBtn.addActionListener(e -> { is3DView = !is3DView; canvas.repaint(); });
        control.add(viewBtn);
        ...
        saveBtn.addActionListener(e -> {
            ...
            if (fc.showSaveDialog(null)==JFileChooser.APPROVE_OPTION) {
                try {
                    ...
                    canvas.paint(g2); g2.dispose();
                    ImageIO.write(img,"PNG",new File(fc.getSelectedFile()+".png"));
                    JOptionPane.showMessageDialog(null,"Saved!");
                } catch(IOException ex){ JOptionPane.showMessageDialog(null,"Save failed"); }
            }
        });
        control.add(saveBtn);
        ...
        delBtn.addActionListener(e -> { rotateX=rotateY=0; isDaytime=true; canvas.repaint(); });
        control.add(delBtn);
