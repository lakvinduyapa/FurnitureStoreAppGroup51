...
public class Stool extends JPanel implements GLEventListener {
    ...
    private float stoolColorR = 0.55f;
    private float stoolColorG = 0.27f;
    private float stoolColorB = 0.07f;
    private boolean isDaytime = true;

    private float roomLength = 2.5f;
    private float roomBreadth = 2.5f;
    private final float roomHeight = 2.5f;
    private String roomShape = "Rectangle";

    private float stoolPosX = 0.0f;
    private float stoolPosZ = 0.0f;
    ...
    public Stool() {
        ...
        canvas.addMouseMotionListener(new MouseMotionAdapter() {
            ...
            public void mouseDragged(MouseEvent e) {
                ...
                stoolPosX = Math.max(-maxX, Math.min(maxX, stoolPosX));
                stoolPosZ = Math.max(-maxZ, Math.min(maxZ, stoolPosZ));

                if (roomShape.equals("L-Shape") && stoolPosX > 0 && stoolPosZ > 0) {
                    if (stoolPosX > stoolPosZ) {
                        stoolPosX = stoolPosZ;
                    } else {
                        stoolPosZ = stoolPosX;
                    }
    ...
    public void display(GLAutoDrawable drawable) {
        ...
        drawStool(gl, stoolColorR, stoolColorG, stoolColorB);
        gl.glFlush();
    }
    ...
    private void drawStool(GL2 gl, float r, float g, float b) {
        float floorY = -roomHeight / 2f;

        float scale = scaleFitEnabled ? Math.min(roomLength, roomBreadth) / 3f : 1.0f;
        gl.glPushMatrix();
        gl.glTranslatef(stoolPosX, 0.0f, stoolPosZ);
        ...
        for (float x = -0.2f; x <= 0.2f; x += 0.4f) {
            for (float z = -0.2f; z <= 0.2f; z += 0.4f) {
                gl.glPushMatrix();
        ...
        gl.glPushMatrix();
        gl.glTranslatef(0.0f, floorY + 1.0f, 0.0f);
        gl.glScalef(0.6f, 0.1f, 0.6f);
        glut.glutSolidCube(1);
        gl.glPopMatrix();
